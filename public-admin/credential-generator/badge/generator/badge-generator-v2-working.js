...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function renderBadgePreview(record) {
    ...
    try {
      ...
      const pngLearnerName =
        pngRenderContainer?.querySelector(
            "#pngLearnerName"
        );

    const pngCredentialType =
    pngRenderContainer?.querySelector(
        "#pngCredentialType"
    );

    const pngProgramCode =
    pngRenderContainer?.querySelector(
        "#pngProgramCode"
    );

    const pngCredentialId =
    pngRenderContainer?.querySelector(
        "#pngCredentialId"
    );
    ...
    pngRenderContainer?.querySelector(
      ...
      if (credentialType) {
        ...
        if (pngCredentialType) {
        pngCredentialType.textContent =
            getDisplayCredentialTitle(record);
        }
